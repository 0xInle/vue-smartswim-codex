...
const REQUIRED_OBJECTS = {
  table: ['public.account_profiles', 'public.account_athletes'],
  rls: ['public.account_profiles', 'public.account_athletes'],
  policy: [
    'Allow authenticated users to read own account profile',
    'Allow authenticated users to insert own account profile',
    'Allow authenticated users to update own account profile',
    'Allow admin read account profiles',
    'Allow authenticated users to read own account athletes',
    'Allow authenticated users to insert own account athletes',
    'Allow authenticated users to update own account athletes',
    'Allow authenticated users to delete own account athletes',
    'Allow admin read account athletes',
  ],
  constraint: [
    'account_profiles_email_check',
    'account_athletes_gender_check',
    'account_athletes_full_name_check',
  ],
  trigger: [
    'account_profiles_touch_updated_at',
    'account_athletes_touch_updated_at',
  ],
  index: [
    'account_profiles_pkey',
    'account_profiles_updated_at_idx',
    'account_athletes_pkey',
    'account_athletes_owner_user_id_idx',
    'account_athletes_updated_at_idx',
  ],
  publication: ['public.account_profiles', 'public.account_athletes'],
}
      ...
      query: `
        with found_objects as (
          ...
          where table_schema = 'public'
            and table_name in ('account_profiles', 'account_athletes')
          ...
          where namespace.nspname = 'public'
            and class.relname in ('account_profiles', 'account_athletes')
          ...
          where policy.schemaname = 'public'
            and policy.tablename in ('account_profiles', 'account_athletes')
          ...
          where index_info.schemaname = 'public'
            and index_info.tablename in ('account_profiles', 'account_athletes')
          ...
          where pubname = 'supabase_realtime'
            ...
            and tablename in ('account_profiles', 'account_athletes')
        )
...
async function main() {
  ...
  try {
    ...
    if (missingObjects.length) {
      console.error('Account profile/athletes contract is incomplete.')
    ...
    console.log(`Account profile/athletes contract is ready in project ${resolvedProjectRef}.`)
    console.log('Checked: tables, RLS, policies, constraints, triggers, indexes, realtime publication.')
  } catch (error) {
    fail(
      error instanceof Error
        ? error.message
        : 'Не удалось проверить Supabase contract для account profile/athletes.',
    )
  }
}
